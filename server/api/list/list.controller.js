...
exports.show = function (req, res, next) {
    ...
    ListModel.find({ listName: listName }, function (err, listData) {
        ...
        listHelper.getList(listName, timestamp, listData[0].settings, req.user, tz, {
            success: items => {
                //console.log(items);
                res.status(200);
...
exports.pull = function (req, res, next) {
 console.log("pull");
    ...
    let returnObj = {
        ts: listHelper.reverseTransformTimestamp(new Date()),
        result: []
    };
    if (req.params) {
        if (req.params.hasOwnProperty('timestamp')) {
    ...
    listHelper.getList('ApplicationLookups', '', '{\"criteria\":{\"listName\": \"AppPullList\"}}', req.user, tz, {
        success: items => {
            var itemNo = 0;
            items.forEach(function (item) {
                if (item.optParam1.opt1 == 'ApplicationLookups') {
                    listHelper.getDirectList(item.optParam1.opt1, timestamp, item.optParam1.opt2, req.user, tz, {
                        success: items1 => {
                    ...
                    listHelper.getList(optParam1.opt1, timestamp, optParam1.opt2, req.user, tz, {
                        success: items1 => {
                            returnObj.result.push(items1);
                            //items1.forEach(function(item1))
                            itemNo++;
