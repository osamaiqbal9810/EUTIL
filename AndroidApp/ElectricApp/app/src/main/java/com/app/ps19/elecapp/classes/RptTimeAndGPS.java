...
public class RptTimeAndGPS {
    public  class TimeEvent{
        ...
        private void loadTimeKey(String _timeKey){
            Date dt= null;
            try {
                dt = Utilities.ConvertToDateTime(_timeKey);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if(dt==null) {
                try {
                    dt = FULL_DATE_FORMAT.parse(_timeKey);
                    timeKey=dt.getTime();
                } catch (Exception e) {
                    e.printStackTrace();
                }
        ...
        if(jp != null){
            for(Task task:jp.getTaskList()){
                if(!task.getStartTime().equals("")){
                    ...
                    eventTask.description=getString(R.string.task)+": " +task.getTitle();
                    ...
                    if(!task.getEndTime().equals("")){
                        ...
                        eventTask1.description=getString(R.string.task)+": " +task.getTitle();
