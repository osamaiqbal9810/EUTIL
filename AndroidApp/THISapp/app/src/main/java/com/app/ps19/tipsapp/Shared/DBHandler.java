...
import static com.app.ps19.tipsapp.Shared.Globals.isDbLocked;
...
*/
    ...
    public int RemoveMsgListItems(List<StaticListItem> array,String orgCode,int status)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        int count=0;
        for(StaticListItem item:array) {
    ...
    public int RemoveMsgListItem(StaticListItem item)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        int count=0;
    ...
    public HashMap<String, String> getAppLookupList(String strListName, String desc, int mode){
    ...
    public HashMap<String, String> getLookupListObj(String strListName, String desc){
        String sql="SELECT CODE, DESCRIPTION,OPTPARAM1 FROM "+ TABLE_APPLICATIONLOOKUPS
                ...
                +(desc.equals("")?"": " AND DESCRIPTION='"+ desc +"'") ;
        SQLiteDatabase db=this.getReadableDatabase();
        Cursor cursor= db.rawQuery(sql,null);
        HashMap<String ,String >list=new HashMap<>();
        if(cursor.moveToFirst()) {
    ...
    public ArrayList<JSONObject> getWorkPlanTemplateList(){
        ...
        while(cursor.moveToNext()){
            try {
                JSONObject jo = new JSONObject(cursor.getString(4));
                JSONObject jp = new JSONObject();
                jp.put("code", jo.optString("_id"));
