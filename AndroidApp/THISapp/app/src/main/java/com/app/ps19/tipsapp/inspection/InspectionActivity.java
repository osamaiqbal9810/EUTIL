...
import com.app.ps19.tipsapp.MaintenanceActivity;
import com.app.ps19.tipsapp.R;
import com.app.ps19.tipsapp.ReportAddActivity;
import com.app.ps19.tipsapp.ReportInboxActivity;
...
import com.app.ps19.tipsapp.ativ.ATIVDefectsActivity;
...
import com.app.ps19.tipsapp.map.InspectionMapActivity;
import com.app.ps19.tipsapp.reports.ReportShowActivity;
import com.app.ps19.tipsapp.unitsGroup.UnitsGroupInspectionActivity;
...
import static com.app.ps19.tipsapp.Shared.Globals.isShowATIV;
...
import static com.app.ps19.tipsapp.Shared.Globals.visitLockOption;
...
import static com.app.ps19.tipsapp.Shared.Utilities.createATextView;
...
public class InspectionActivity extends AppCompatActivity implements OnLocationUpdatedListener, StopInspectionFragment.StopDialogListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Check if assets are not available
        if(getSelectedTask().getWholeUnitList().size() == 0){
            Toast.makeText( InspectionActivity.this,getResources().getText(R.string.asset_available), Toast.LENGTH_SHORT).show();
            finish();
        }
...
/*                    ViewGroup.LayoutParams parm = inspectionExpandableListView.getLayoutParams();
    ...
    public class InspectionUnitsAdapter extends BaseExpandableListAdapter
    {
        ...
        public int getChildrenCount(int groupPosition)
        {
            ...
            DUnit selected = (DUnit) getGroup(groupPosition);
            if(selected.getUnit().isGroupMember()){
                return 0;
            }
            ArrayList<UnitsTestOpt> testList = selected.getUnit().getTestFormList();
                                 ...
                                 View convertView, ViewGroup parent)
        {
            ...
            if(selectedJPlan!=null){
                ImageView imgEquipmentList=tv.findViewById(R.id.imgEquipmentList);
                TextView tvMilPostLimits=tv.findViewById(R.id.tvMilPostLimits);
                ImageView imgMpEndValue=tv.findViewById(R.id.imgMpEndValue);

                TextView tvAtivDefCounter = tv.findViewById(R.id.ativ_issue_count_new);
                FrameLayout flAtivDef = tv.findViewById(R.id.fl_ativ_defects);

                TextView tvMilPostStart=tv.findViewById(R.id.tvGroupRow);
                TextView tvMilPostEnd=tv.findViewById(R.id.tvGroupColumn);
                ...
                if(currUnit.getUnit().getChildren().size()>0){
                    imgEquipmentList.setVisibility(VISIBLE);
                }else{
                    imgEquipmentList.setVisibility(GONE);
                }
                flAtivDef.setVisibility(GONE);
                if(isShowATIV){
                    if(currUnit.getUnit().getAtivDefects()!=null && currUnit.getUnit().getAtivDefects().size()!=0){
                        flAtivDef.setVisibility(VISIBLE);
                        tvAtivDefCounter.setText(String.valueOf(currUnit.getUnit().getAtivDefects().size()));
                    }
                }
                flAtivDef.setOnClickListener(v -> {
                    if(activeSession == null){
                        Toast.makeText(getApplicationContext(), getString(R.string.no_active_session), Toast.LENGTH_SHORT).show();
                    }
                    else {
                        Intent intent = new Intent(InspectionActivity.this, ATIVDefectsActivity.class);
                ...
                int oldIssueCount=getOldIssueCount(unitOpt.getUnit());
                newIssueCount=getNewIssueCount(unitOpt.getUnit());
                ...
                String title=getUnitDescription(unitOpt.getUnit());
                        ...
                        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                            ...
                            public boolean onMenuItemClick(MenuItem item) {
                                ...
                                switch (item.getItemId()) {
                                    ...
                                    case R.id.mnuFormsTests:
                                        if (selectedUnit.getCountFormWithTests() > 0) {
                                    ...
                                    case R.id.mnuForms:
                                        ...
                                        if (selectedUnit.getCountFormWithoutTests()> 0) {
                                            AppFormDialogFragment dialogFragment = new AppFormDialogFragment(false,false);
                                            dialogFragment.show(getSupportFragmentManager(), "appFormDialogNoTests");
                                        } else {
                                            Toast.makeText(InspectionActivity.this, getString(R.string.no_form_available), Toast.LENGTH_SHORT).show();
                                        }
                ...
                imgEquipmentList.setOnClickListener(v->{
                    DUnit dUnit1 = (DUnit) getGroup(groupPosition);
                    Intent intent1 = new Intent(InspectionActivity.this, UnitsGroupInspectionActivity.class);
                    selectedUnit = dUnit1.getUnit();
                    selectedDUnit = dUnit1;
                ...
                ivAddDefect.setOnClickListener(v -> {
                    if(activeSession == null){
                        ...
                        Intent intent1 = new Intent(InspectionActivity.this, UnitsGroupInspectionActivity.class);
                        DUnit dUnit1 = (DUnit) getGroup(groupPosition);
                        if(intent1!=null && dUnit1.getUnit().isGroupMember()) {
                            selectedUnit = dUnit1.getUnit();
                            selectedDUnit = dUnit1;
                            startActivity(intent1);
                            return;
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    if(unitOpt.getUnit().isGroupMember()){
                        view.setBackgroundResource(R.drawable.baseline_workspaces_24);
                    }
                    view.setBackgroundTintList (ColorStateList.valueOf(color));
                }
                //String label=""+ template.getSortOrder()+". "+ template.getTitle();
                //            textView.setText(label);
                textView.setText(getUnitDescription(unitOpt.getUnit()));
                if(getSelectedTask().isYardInspection() || appName.equals(Globals.AppName.SCIM)){
                    imgObserve.setVisibility(GONE);
                    imgTraverse.setVisibility(GONE);
                }
           ...
           /* if(currUnit.getUnit().getAssetTypeObj().isMarkerMilepost()){
                ...
                tvMarkerStart.setText(getPrefixMp(unitOpt.getUnit().getStart()));
                tvMarkerEnd.setText(getPrefixMp(unitOpt.getUnit().getEnd()));
        ...
        private int getUnitColor(Units unit){
            ...
            if(unit.isGroupMember()){
                color=unit.getUnitsGroup().getGroupColor();
            }
            return color;
        }
        private int getNewIssueCount(Units unit){
            ...
            if(unit.isGroupMember()){
                issueCount=unit.getUnitsGroup().getNewIssueCount();
            }
            return issueCount;
        ...
        private int getOldIssueCount(Units unit){
            ...
            //selectedJPlan.getTaskList().get(0).getUnitDefectsListByID(currUnit.getUnit().getUnitId()).size()
            if(unit.isGroupMember()){
                issueCount=unit.getUnitsGroup().getOldIssueCount();
            }
            return issueCount;
        ...
        private String getUnitDescription(Units unit){
            ...
            if(unit.isGroupMember()){
                title=unit.getAttributes().getGroup();
            }
            return title;
        }
        @Override
    ...
    private void showSwitchingSessionDialog(final Context context, ViewGroup parent, Units uSelection, String mode){
        ...
        String _lMsg = uSelection.getDescription() + getString(R.string.from_part1) + getPrefixMp(uSelection.getStart())+ getString(R.string.to_part2) + getPrefixMp(uSelection.getEnd());
        ...
        spObserveTracks.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(unitId.equals("0")){
                    ...
                    lMsg = observeTracks.get(position).getDescription() + getString(R.string.from_part1) + getPrefixMp(observeTracks.get(position).getStart()) + getString(R.string.to_part2) + getPrefixMp(observeTracks.get(position).getEnd());
                }
                tvObserveTrackLimit.setText(lMsg);
                //Toast.makeText(TaskInspectionActivity.this, tracks.get(position).getDescription(), Toast.LENGTH_SHORT).show();
            }
        ...
        String rangeMsg = "<b>" + prefix + " " + "</b> " + getPrefixMp(getSelectedTask().getMpStart()) + getString(R.string.to_part2) + "<b>" + prefix + " " + "</b> " + getPrefixMp(getSelectedTask().getMpEnd());
...
//                } else{
    ...
    private void updateSessionDetails(boolean... isNewSession ) {
        ...
        if (activeSession == null || !is_new_session) {
            ...
            tvStartMP.setText(getPrefixMp(activeSession.getStart()));
            tvEndMP.setText(getPrefixMp(activeSession.getEnd()));
