/*
package com.app.ps19.scimapp.geofencing.utils;
...
//import com.app.ps19.scimapp.location.Interface.Store;

public class GeofencingStore implements Store<GeofenceModel> {

    public static final String GEOFENCE_PREFERENCES_FILE = "GEOFENCING_STORE";
    ...
    public GeofencingStore(@NonNull Context context) {
        preferences = context.getSharedPreferences(GEOFENCE_PREFERENCES_FILE, Context.MODE_PRIVATE);
    }
...
/*        editor.putLong(getFieldKey(id, EXPIRATION_ID), geofenceModel.getExpiration());
    ...
    public GeofenceModel get(String id) {
        if (preferences != null &&
                preferences.contains(getFieldKey(id, LATITUDE_ID)) &&
                preferences.contains(getFieldKey(id, LONGITUDE_ID)))
        {
