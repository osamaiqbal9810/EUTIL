...
import static com.app.ps19.hosapp.Shared.Utilities.makeVoiceAvailableEx;
...
public class reportVoiceAdapter extends RecyclerView.Adapter<reportVoiceAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if(voiceList.get(position).getVoiceName() == null || voiceList.get(position) == null || voiceList.get(position).getVoiceName().equals("")){
            ...
            final File audioFile = new File(VOICE_PATH + voiceList.get(position).getVoiceName());
            ...
            //
            holder.tvVoiceDuration.setText(getDuration(new File(VOICE_PATH + voiceList.get(position).getVoiceName())));
                ...
                public void onClick(final View v) {
                    ...
                    try {
                        ...
                        if (audioFile.isFile() && audioFile.exists()) {
                            ...
                            mp = MediaPlayer.create(context, Uri.parse(VOICE_PATH + voiceList.get(position).getVoiceName()));//mp.setDataSource(voiceList.get(position).getVoiceName());
                            mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener()
                            {
                                @Override
                                public void onCompletion(MediaPlayer mPlayer)
