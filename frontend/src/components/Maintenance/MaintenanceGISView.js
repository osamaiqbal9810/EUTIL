...
function getMPLocation(Locs) {
  let l = !Locs || Locs.length == 0 ? { start: 0, end: 0 } : Locs[0].type === 'Milepost' ? Locs[0] : Locs.length > 1 && Locs[1].type === 'Milepost' ? Locs[1] : { start: 0, end: 0 };//null;
  return l;
}
...
function processMaintenances(maintenances, props) {
  let textcolor = 'black', color = 'black';
  let checkedMs = [];
  let colors = ['#E53935', '#D81B60', '#8E24AA', '#5E35B1', '#3949AB', '#1E88E5', '#039BE5', '#00ACC1', '#00897B',
    '#43A047', '#7CB342', '#C0CA33', '#FDD835', '#FFB300', '#FB8C00', '#F4511E', '#6D4C41', '#757575', '#546E7A']//['black', 'green', 'red', 'blue','cyan', 'magenta'];
  let textStyle = {
  ...
  let maintenanceLines = maintenances.map((val, index) => {
    ...
    textcolor = color;
    {
      ...
      //if(loc !== null)
      {
        ...
        let msg = (<div><h4 style={headingStyle}>{val.mrNumber}</h4>
          <div style={textStyle}>
            <div><strong>{val.lineName ? val.lineName : ''}</strong></div>
            <div><strong>{languageService('created at')}:</strong> {formatDate(val.createdAt)}</div>
            {/*<div><strong>{languageService('Location')}:</strong> {format2Digits(loc.start)} to {format2Digits(loc.end)}</div>*/}

            {locationRender(val.location)}
            ...
            <Link style={{ float: "right", textDecoration: "none" }} to={url}><span style={{
              ...
              fontFamily: "Arial",
              letterSpacing: "0.3px"
            }}>{languageService('View')}</span></Link>

          </div>
        </div>);

        let data = { start: +loc.start, end: +loc.end, text: formatDate(val.createdAt), color: color, offset: 0, msg: msg, width: 5, visible: true, _id: val.mrNumber };
...
export const MaintenanceGISView = (props) => {
  ...
  let lineGroups = [];
  let selectedLine = {}, selectedMaintenance = null; //, defaultCenter=[0,0]; // this needs to be calculated only once not repeatedly

  for (let baseLineId of lines) {
    let lineMaintenances = maintenancesByLine[baseLineId];
    let baseLine = props.linesList.find((l) => { return l._id == baseLineId; });
