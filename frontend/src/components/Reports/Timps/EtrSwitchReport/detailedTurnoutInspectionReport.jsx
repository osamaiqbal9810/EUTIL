...
import DetailedTurnoutInspectionView from "./detailedTurnoutInspectionReportView";
import moment from "moment";
import { switchTurnoutsIncluded, switchFormCodeAllowed } from "../../../../AssetTypeConfig/Reports/ETRDetailedTurnoutInspectionReport";
import { LocPrefixService } from "../../../LocationPrefixEditor/LocationPrefixService";
export default class DetailedTurnoutInspection extends Component {
  constructor(props) {
    super(props);
    ...
    if (task && task.units) {
      ...
      task.units.forEach((asset) => {
        if (_.find(switchTurnoutsIncluded, (item) => item == asset.assetType)) {
          let etrSwitchForm = _.find(asset.appForms, (form) => form && switchFormCodeAllowed(form.id));
          ...
          if (etrSwitchForm || assetIssues.length > 0) {
            let assetData = {
              ...
              issues: assetIssues,
              location: locName,
              form: etrSwitchForm ? etrSwitchForm.form : null,
            };
  ...
  render() {
    ...
    if (!atleastOne) detailedTurnoutView = <DetailedTurnoutInspectionView />;
    else {
      detailedTurnoutView = this.state.assetsData.map((assetData) => {
        return (
          <DetailedTurnoutInspectionView
