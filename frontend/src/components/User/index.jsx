...
class User extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.userActionType !== prevProps.userActionType) {
      if (USER_LOADING_START_ACTIONS.includes(this.props.userActionType)) this.setState({ spinnerLoading: true });

      if (USER_LOADING_STOP_ACTIONS.includes(this.props.userActionType)) this.setState({ spinnerLoading: false });

      if (USER_TOAST_MESSAGES_ACTIONS.includes(this.props.userActionType)) {
        if (this.props.userActionType.includes("FAILURE"))
          this.showToastError(
            languageService(this.props.userErrorMessage) || languageService(getWarningMessage(this.props.userActionType)),
          );
      ...
      if (USER_LIST_TO_UPDATE.includes(this.props.userActionType)) this.props.userListRequest();
    ...
    if (this.props.userDetailActionType !== prevProps.userDetailActionType) {
      if (USER_LOADING_START_ACTIONS.includes(this.props.userDetailActionType)) this.setState({ spinnerLoading: true });

      if (USER_LOADING_STOP_ACTIONS.includes(this.props.userDetailActionType)) this.setState({ spinnerLoading: false });
    }

    if (this.state.selectedUserId !== prevState.selectedUserId) {
      const loggedInUser = localStorage.getItem("loggedInUser");
      const currentUser = JSON.parse(loggedInUser);
      if (this.state.selectedUserId) this.props.getUserWithDetail(this.state.selectedUserId);
      else this.props.getUserWithDetail(currentUser._id);
    ...
    return (
      <Row style={{ margin: "0px" }}>
        ...
        {permissionCheck("USER", "view") && (
          <Col md={5}>
            <div style={{ background: "var(--fifth)", borderRadius: "5px" }}>
...
let UserContainer = CRUDFunction(User, "userComp", actionOptions, variables, ["userReducer", "templateHelperReducer"]);
